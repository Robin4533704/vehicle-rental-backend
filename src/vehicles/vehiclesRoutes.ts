...
router.post("/", (req: any, res) => {
  ...
  if (exist) return res.status(400).json({ message: "Vehicle with this registration already exists" });
...
router.put("/:vehicleId", (req, res) => {
  ...
  if (daily_rent_price) vehicle.daily_rent_price = daily_rent_price;
...
export const getAllVehicles = async () => { const result = await pool.query("SELECT * FROM vehicles ORDER BY id ASC"); return result.rows; };
